...
void initialize_vector(Vector *vector) {
    // seed randomn on cur time
    srand(time(NULL));
    do {
        vector->x = (rand() % 3) - 1;
        vector->y = (rand() % 3) - 1;
    } while (vector->x == 0 || vector->y == 0); // generate canted or non planar heading
    vector->magnitude = 1;
}
...
void render_ascii_art(Point points[], int num_points, int offsetX, int offsetY) {
    ...
    // render the points of the offset ASCII art
    for (int i = 0; i < num_points; i++) {
...
int main() {
    ...
    // convert 2d string to 1d point array
    ...
    while (1) {
        ...
        // angle of reflection on collisions
        for (int i = 0; i < num_points; i++) {
